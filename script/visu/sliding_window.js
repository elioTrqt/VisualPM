import { Graphic, GraphicList, Arrow, Vector } from "../graphics.js";
export class SlidingWindow extends Graphic {
    text;
    pattern;
    arrow;
    current_step;
    constructor(parent, pos, anim_speed, w, text, pattern) {
    ...
    update(step) {
        ...
        if (step.arrow) {
            if (this.arrow) {
                ...
                // Create the arrow
                this.arrow = new Arrow(this.group, this.pattern.get_cell_pos(step.arrow.position.start, 0, -1), this.text.get_cell_pos(step.arrow.position.end, 0, 1), this.speed, step.arrow.color);
            }
        }
        else {
            // Hide the arrow
