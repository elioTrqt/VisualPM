...
import { Graphic, GraphicList, DynamicSection, SlidingWindow } from "./graphics.js";
import { Vector } from "./vector.js";
class SuffList extends GraphicList {
    step;
    done;
    data;
    pattern;
...
class DList extends GraphicList {
    step;
    done;
    data;
    data_from;
    ...
    index;
    suff;
    constructor(parent, pos, w, pattern, index, suff) {
        super(parent, pos, w, new Array(pattern.values.length).fill(null));
        this.step = 0;
        this.done = false;
        this.pattern = pattern;
...
class DTable extends Graphic {
    ...
    static get_canvas_size(pattern) {
...
class RTable extends Graphic {
    ...
    constructor(parent, pos, w, pattern, improved = false) {
        super(parent, pos);
        const offset_pos = pos.add(new Vector(100, 0));
        this.index = new GraphicList(this.group, offset_pos, w, Array.from({ length: pattern.length }, (_, index) => index + 1));
        this.pattern = new GraphicList(this.group, offset_pos.add(new Vector(0, w)), w, pattern.split(""));
    ...
    static get_canvas_size(pattern, improved, width) {
        const chars = ['...'];
        const freqs = new Map;
        freqs.set('...', 1);
...
export class AlgBM {
    ...
    state = 'check';
    constructor(container, pattern, text, improved) {
