import { RTableState, dictRowUpdate } from "../visu/bm/right.js";
import { DTableState } from "../visu/bm/decal.js";
...
export function init_decal(pattern, suff) {
    const m = pattern.length;
    ...
    const updated = new Array(m + 1).fill(null);
    for (let j = 1; j <= m; j++) {
        decal[m - suff[j]] = m - j;
...
export function init_right(pattern, improved) {
    const right = new Map();
    const steps = [];
